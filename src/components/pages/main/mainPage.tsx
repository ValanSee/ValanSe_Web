...
const MainPage = () => {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-[#F0F0F0] px-4 py-6">
      ...
      {/* 밸런스 게임 만들기 */}
      <button className="my-6 py-3 px-6 bg-white rounded-xl shadow-md font-semibold
      ...
      <div className="flex justify-around w-full max-w-md mb-6 gap-4">
        {categories.map((c) => (
          <div key={c.label} className={`p-4 rounded-lg border shadow-md text-left py-6 w-full ${
            selectedOption === c.label ? 'bg-black text-white' : 'bg-white text-gray-800'
          }`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around py-2">
        {navItems.map((label, i) => (
          <div key={i} className="text-center text-sm text-gray-700 flex flex-col items-center justify-around">
